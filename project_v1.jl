...
TC3inhib_network = (
    ...
    inh_SST=IFParameter(      # SST：slower
        τm=200pF / 10nS,
        El=-70mV,
        Vt=-53mV,
        Vr=-70mV,
    ...
    inh_VIP=IFParameter(      # VIP：medium
        τm=150pF / 10nS,
        El=-70mV,
        Vt=-53mV,
        Vr=-70mV,
    ...
    connections=(
        ...
        # from CortExc
        CortExc_to_CortExc=(p=0.02, μ=2nS, rule=:Fixed),
        CortExc_to_CortPv=(p=0.02, μ=2nS, rule=:Fixed),
        CortExc_to_ThalExc=(p=0.02, μ=2nS, rule=:Fixed),  # <<< ADD: Cortex→Thalamus feedback
        # from CortPv
...
function network(config)
    ...
    synapses = (
        ...
        PV_to_SST=SpikingSynapse(PV, SST, :gaba, conn=connections.CortPv_to_CortSst), SST_to_CE=SpikingSynapse(SST, CE, :gaba, conn=connections.CortSst_to_CortExc),
